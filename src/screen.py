...
class Screen:
    ...
    def draw_box(self, lt_pos: Vec2, rd_pos: Vec2, style: int = 0):
        ...
        x1, y1 = int(lt_pos.x) * 2, int(lt_pos.y)
        x2, y2 = int(rd_pos.x) * 2, int(rd_pos.y)
        if x1 >= x2 or y1 >= y2:
    ...
    ('┘', '╗'): '╣', ('╗', '┘'): '╣',
        ...
        for x in range(x1 + 1, x2):
            old_char_top = chr(self.get_char(Vec2(x / 2, y1)))
            old_char_bottom = chr(self.get_char(Vec2(x / 2, y2)))
            self.set_char(Vec2(x / 2, y1), merge_chars(old_char_top, box_chars['h']))
            self.set_char(Vec2(x / 2, y2), merge_chars(old_char_bottom, box_chars['h']))

        # Рисуем вертикальные линии
        for y in range(y1 + 1, y2):
            old_char_left = chr(self.get_char(Vec2(x1 / 2, y)))
            old_char_right = chr(self.get_char(Vec2(x2 / 2, y)))
            self.set_char(Vec2(x1 / 2, y), merge_chars(old_char_left, box_chars['v']))
            self.set_char(Vec2(x2 / 2, y), merge_chars(old_char_right, box_chars['v']))

        # Рисуем углы (с учетом возможных наложений)
        old_tl = chr(self.get_char(Vec2(x1 / 2, y1)))
        old_tr = chr(self.get_char(Vec2(x2 / 2, y1)))
        old_bl = chr(self.get_char(Vec2(x1 / 2, y2)))
        old_br = chr(self.get_char(Vec2(x2 / 2, y2)))

        self.set_char(Vec2(x1 / 2, y1), merge_chars(old_tl, box_chars['tl']))
        self.set_char(Vec2(x2 / 2, y1), merge_chars(old_tr, box_chars['tr']))
        self.set_char(Vec2(x1 / 2, y2), merge_chars(old_bl, box_chars['bl']))
        self.set_char(Vec2(x2 / 2, y2), merge_chars(old_br, box_chars['br']))
    ...
    def clear(self):
        ...
        self.draw_box(Vec2(0, 0), self.size - Vec2(0.5, 1))
    
    def is_on_screen(self, pos: Vec2):
        return pos.x < 0 or round(pos.x * 2) >= self.width * 2 or int(pos.y) < 0 or int(pos.y) >= self.height